      ...
      <div className='w-full p-6 md:p-10 justify-between space-y-8 md:space-y-12 '>
        <div className='w-full flex justify-between space-x-4'>
          <button onClick={() => { if (!isLoading) setMode("owner"); }}
            className="w-fit px-4 py-2 md:px-6 md:py-3 text-white font-semibold bg-gradient-to-r from-indigo-700 via-purple-500 to-pink-600 rounded-lg shadow-lg hover:scale-105 duration-200 hover:drop-shadow-2xl hover:shadow-[#7dd3fc] hover:cursor-pointer"
            disabled={isLoading}>
            Switch to owner's dashboard
          </button>
          <div className="flex">
            <button onClick={clearLogs}
              className={`w-fit border hover:scale-95 duration-300 relative group cursor-pointer text-sky-50 overflow-hidden h-[44px] w-28 md:w-36 my-auto rounded-md bg-sky-200 p-2 flex justify-center items-center font-extrabold ${(!checkoutSuccessful && lastSuccessfulContractId === -1) || isLoading ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={isLoading}>
        ...
        <div className='flex flex-col md:flex-row md:space-x-10 w-full justify-between'>
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <button onClick={() => { if (!isLoading) setModalCheckoutOpened(true); }}
               className={`w-fit border hover:scale-95 duration-300 relative group cursor-pointer text-sky-50 overflow-hidden h-[44px] w-28 md:w-36 my-auto rounded-md bg-sky-200 p-2 flex justify-center items-center font-extrabold ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={isLoading}>
                <div className="absolute right-32 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-40 h-40 rounded-full group-hover:scale-150 duration-500 bg-sky-900"></div>
              <div className="absolute right-2 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-32 h-32 rounded-full group-hover:scale-150 duration-500 bg-sky-800"></div>
              <div className="absolute -right-12 top-4 group-hover:top-1 group-hover:right-2 z-10 w-24 h-24 rounded-full group-hover:scale-150 duration-500 bg-sky-700"></div>
              <div className="absolute right-20 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-16 h-16 rounded-full group-hover:scale-150 duration-500 bg-sky-600"></div>
        ...
        <div className='flex flex-col md:flex-row md:space-x-10 w-full justify-between'>
          ...
          <div className="flex items-center justify-center space-x-3">
            ...
            <button onClick={() => { if (!isLoading) setModalReviewLogOpened(true); }}
              ...
              className={`border hover:scale-95 duration-300 relative group cursor-pointer text-sky-50 overflow-hidden h-[44px] w-28 md:w-32 my-auto rounded-md bg-sky-200 p-2 flex justify-center items-center font-extrabold ${(!checkoutSuccessful && lastSuccessfulContractId === -1) || isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
                <div className="absolute right-32 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-40 h-40 rounded-full group-hover:scale-150 duration-500 bg-sky-900"></div>
                <div className="absolute right-2 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-32 h-32 rounded-full group-hover:scale-150 duration-500 bg-sky-800"></div>
                <div className="absolute -right-12 top-4 group-hover:top-1 group-hover:right-2 z-10 w-24 h-24 rounded-full group-hover:scale-150 duration-500 bg-sky-700"></div>
                <div className="absolute right-20 -top-4 group-hover:top-1 group-hover:right-2 z-10 w-16 h-16 rounded-full group-hover:scale-150 duration-500 bg-sky-600"></div>
